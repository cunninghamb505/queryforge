...
def create_favorite_query(name: str, sql_text: str, connection_id: int | None = None) -> FavoriteQuery:
    with SessionLocal() as session:
        fav = FavoriteQuery(name=name, sql_text=sql_text, connection_id=connection_id)
        session.add(fav)
        session.commit()
        session.refresh(fav)
        return fav
